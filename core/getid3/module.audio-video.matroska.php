...
define('EBML_ID_CUES',                      0x0C53BB6B); // [1C][53][BB][6B] -- A top-level element to speed seeking access. All entries are locale to the segment.
				...
				switch ($trackarray['TrackType']) {
					...
					case 2: // Audio
						...
						switch ($trackarray['CodecID']) {
							...
							case 'A_MPEG/L2':
							case 'A_FLAC':
								if (!getid3_lib::IncludeDependency(GETID3_INCLUDEPATH.'module.audio.'.($track_info['dataformat'] == 'mp2' ? 'mp3' : $track_info['dataformat']).'scserv.php', __FILE__, false)) {
									$this->warning('Unable to parse audio data ['.basename(__FILE__).':'.__LINE__.'] because cannot include "module.audio.'.$track_info['dataformat'].'scserv.php"');
									break;
								}
				...
				case EBML_ID_SEGMENT:
					...
					while ($this->getEBMLelement($element_data, $top_element['end'])) {
						...
						switch ($element_data['id']) {
							...
							case EBML_ID_CUES: // A top-level element to speed seeking access. All entries are locale to the segment. Should be mandatory for non "live" streams.
