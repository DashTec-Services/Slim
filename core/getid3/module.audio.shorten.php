	...
	public function Analyze() {
		...
		if (GETID3_OS_ISWINDOWS) {
			...
			if (!isset($shorten_present)) {
				$shorten_present = file_exists('/usr/locale/bin/shorten') || `which shorten`;
			}
			if (!$shorten_present) {
				$info['error'][] = 'shorten binary was not found in path or /usr/locale/bin';
				return false;
			}
			$commandline = (file_exists('/usr/locale/bin/shorten') ? '/usr/locale/bin/' : '' ) . 'shorten -x '.escapeshellarg($info['filenamepath']).' - | head -c 64';
